...
namespace FV.Verification.Eth
{
    public class EthInteraction
    {
        ...
        public string NewReceipt(string fileId, string fileHash, string metadataHash, string verifiedBy)
        {
            // deploy new receipt contract to blockchain
            // must run sync
            string receiptAddress = DeployReceipt.SendRequestAsync(this, fileId, fileHash, metadataHash, verifiedBy).GetAwaiter().GetResult();

            // get state of receipt
            // must run sync
            int state = GetState.SendRequestAsync(this, receiptAddress).GetAwaiter().GetResult();
        ...
        public async Task<History> GetHistory(string fileId)
        {
            ...
            for (int i = 0; i < numReceipts; ++i)
            {
                ...
                string verifiedBy = await GetVerifiedBy.SendRequestAsync(this, receiptAddress);
